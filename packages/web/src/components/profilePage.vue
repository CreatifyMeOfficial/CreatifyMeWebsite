...
async function updateUser(e) {
  ...
  try {
    ...
    if (response.status === 200) {
      createNotification('User info was updated successfully', notificationTypes.Success, 5)
    }
  } catch {
    return
  }
...
async function updateImage(formData) {
  try {
    ...
    if (response.status === 200) {
      await loadUser()
      userStore.fetchUser()
      createNotification('Profile image was updated successfully', notificationTypes.Success, 5)
    }
  } catch {
    return
  }
