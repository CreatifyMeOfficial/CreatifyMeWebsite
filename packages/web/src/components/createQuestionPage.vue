...
async function createQuestion() {
  if (
    ...
    createNotification(
      'Please fill all the fields and select a value for each test!',
  ...
  try {
    const response = await questionsApi.createQuestion({
      ...
      MBTIAttribute: selectedMpti.value,
      HollandAttribute: selectedHolland.value,
    })
    if (response.status === 201) {
      createNotification('Question created successfully!', notificationTypes.Success, 5)
    }
    englishQuestion.value = ''
    arabicQuestion.value = ''
    selectedMpti.value = ''
...
<template>
  ...
  <div class="question-container">
    ...
    <!-- MPTI personality type selection section -->
    <div class="mpti-part content">
      <span>MPTI :</span>
      <table>
        <!-- MPTI trait headers -->
        <tr class="row1">
        ...
        <!-- Radio buttons for selecting MPTI traits -->
        <tr class="row2">
          <td><input type="radio" value="J" name="mpti" v-model="selectedMpti" /></td>
          <td><input type="radio" value="P" name="mpti" v-model="selectedMpti" /></td>
          <td><input type="radio" value="T" name="mpti" v-model="selectedMpti" /></td>
          <td><input type="radio" value="F" name="mpti" v-model="selectedMpti" /></td>
          <td><input type="radio" value="N" name="mpti" v-model="selectedMpti" /></td>
          <td><input type="radio" value="S" name="mpti" v-model="selectedMpti" /></td>
          <td><input type="radio" value="E" name="mpti" v-model="selectedMpti" /></td>
          <td><input type="radio" value="I" name="mpti" v-model="selectedMpti" /></td>
        </tr>
      </table>
    </div>
...
/* Common styling for MPTI and Holland sections */
