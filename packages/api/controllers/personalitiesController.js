...
const GetPersonalities = async (req, res) => {
  ...
  const personalitiesTotal = await Personality.countDocuments({});
...
const AddTagToPersonality = async (req, res) => {
  ...
  if (!personality) {
    throw new NotFoundError("This personality doesn't exist!");
  }

  if (personality.tags.includes(tagId)) {
    return res.status(statusCodes.BAD_REQUEST).json({ msg: 'This personality has this tag already!' });
  }
...
const RemoveTagFromPersonality = async (req, res) => {
  ...
  if (!personality.tags.includes(tagId)) {
    return res.status(statusCodes.NOT_FOUND).json({ msg: "This personality doesn't have this tag!" });
  }
